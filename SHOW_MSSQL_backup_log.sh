...
query="SELECT name, ip, user, pwd, os, frequency, save_path, location, type, project 
       FROM $DB_MNT.servers 
       WHERE active = 1 
         AND type = 'MSSQL' 
...
mysql -u"$DB_USER" -p"$DB_PASS" --batch -se "$query" $DB_MNT | while IFS=$'\t' read_fields SERVER SERVERIP WUSER WUSERP OS SAVE_PATH LOCATION TYPE PROJECT
do
    echo "============================================================================================================"
    ...
    for DATE in "$TEST_DATE" "$TEST_DATE2" "$TEST_DATE3"; do
        ...
        if [[ -z "$DB_FOLDERS" ]]; then
            echo "No database folders found under $BACKUP_PATH"
            continue
        fi
