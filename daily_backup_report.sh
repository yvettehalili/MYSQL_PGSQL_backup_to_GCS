...
appendBarGraphSection() {
...

    {
        ...
        echo "    <div style='display: flex; flex-direction: column; align-items: flex-start; height: auto; border: 1px solid #ddd; padding: 10px;'>"
        ...
        mysql --defaults-file=/etc/mysql/my.cnf --defaults-group-suffix=bk -u"${DB_USER}" -p"${DB_PASS}" -D"${DB_NAME}" -e "${query}" --batch --skip-column-names 2>>"${LOG_FILE}" | while IFS=$'\t' read -r Server size size_name Location DB_engine OS; do
            ...
            percentage=$(echo "${sizeValue}" | awk -v maxSize_MB="${MAX_SIZE_MB}" '{print ($1 / maxSize_MB) * 100}')

            echo "    <div style='display: flex; align-items: center; margin-bottom: 20px;'>"
            echo "      <div style='width: 300px; font-weight: bold; color: #4B286D; margin-right: 10px; writing-mode: vertical-rl; text-orientation: mixed; transform: rotate(180deg); text-align: right;'>${Server}</div>"
            echo "      <div style='height: 30px; width: 100%; background-color: #ddd; border-radius: 10px; overflow: hidden; border: 1px solid #00C853; position: relative;'>"
            echo "        <div style='height: 100%; border-radius: 10px; padding: 0 10px; color: white; line-height: 30px; transition: width 0.3s; display: flex; align-items: center; justify-content: flex-end; font-weight: bold; background-color: #4B286D; width: ${percentage}%;'><span style='position: absolute; right: 10px;'>${sizeValue} MB</span></div>"
            echo "      </div>"
            echo "    </div>"
        done
        echo "      </div>"
