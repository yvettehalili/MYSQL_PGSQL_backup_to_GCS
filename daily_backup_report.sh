...
appendSection() {
...

    {
        ...
        mysql --defaults-file=/etc/mysql/my.cnf --defaults-group-suffix=bk -u"${DB_USER}" -p"${DB_PASS}" -e "${query}" --batch --skip-column-names 2>>"${LOG_FILE}" | while IFS=$'\t' read -r No Server size size_name Location DB_engine OS Error; do
            ...
            echo "        <div style='height: 100%; border-radius: 10px; padding: 0 10px; color: white; line-height: 30px; transition: width 0.3s; display: flex; align-items: center; justify-content: flex-end; font-weight: bold; background-color: #4B286D; width: ${percentage}%;'><span style='position: absolute; right: 10px;'>${size} ${size_name}</span></div>"
            echo "      </div>"
            echo "    </div>"
        done
    } >> "${emailFile}"
...
emailFile="${DIR}/daily_backup_report.html"
{
    ...
    echo "    h1 { color: #4B286D; text-align: center; border-bottom: 2px solid #4B286D; padding-bottom: 10px; }"
    echo "    h2 { margin-top: 40px; border-bottom: 1px solid #4B286D; padding-bottom: 10px; color: #00C853; }"
    ...
    echo "    .footer a:hover { color: #00C853; }"
    echo "  </style>"
    echo "</head>"
    echo "<body>"
