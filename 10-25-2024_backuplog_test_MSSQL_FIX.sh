...
query="SELECT name, ip, user, pwd, os, frequency, save_path, location, type, bucket, project FROM ti_db_inventory.servers WHERE active=1 ORDER BY location, type"
...
do
    ...
    if [[ "$TYPE" == "MSSQL" ]]; then
        ...
        for FILE in $FILES; do
            ...
            FILENAMES+=("$FILENAME")
            DB_NAME=$(basename $(dirname $(dirname "$FILE")))
