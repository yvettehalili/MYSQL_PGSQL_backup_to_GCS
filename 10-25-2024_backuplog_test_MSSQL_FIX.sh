        ...
        MYSQL|PGSQL)
            ...
            if [[ -n "$FILES" ]]; then
                for FILE in $FILES; do
                    ...
                    # Print data to be inserted into backup_log
                    echo "Inserting into backup_log: date=$TEST_DATE, server=$SERVER, size=$fsize, file=$FILE"
                    ...
                    # Print data to be inserted into daily_log
                    echo "Inserting into daily_log: date=$TEST_DATE, server=$SERVER, database=$DATABASE, size=$fsize, state=$STATE, file=$FILENAME"
                    DQUERY="INSERT INTO daily_log (backup_date, server, \`database\`, size, state, last_update, fileName) VALUES ('$TEST_DATE', '$SERVER', '$DATABASE', $fsize, '$STATE', '$endcopy', '$FILENAME');"
                    echo "DQUERY: $DQUERY"
                    mysql -u"$DB_USER" -p"$DB_PASS" $DB_MAINTENANCE -e "$DQUERY" || { echo "Error executing query: $DQUERY"; exit 1; }
                done
            else
                echo "No files found for $SERVER in bucket $BUCKET and path $BACKUP_PATH"
            fi
            ;;
        MSSQL)
            ...
            for DATE in "$TEST_DATE" "$TEST_DATE2" "$TEST_DATE3"; do
                ...
                FILES="$FILES $DIFF_FILES $FULL_FILES"
            ...
            if [[ -n "$FILES" ]]; then
                for FILE in $FILES; do
                    ...
                    # Print data to be inserted into daily_log
                    echo "Inserting into daily_log: date=$TEST_DATE, server=$SERVER, database=$DB_NAME, size=$fsize, state=$STATE, file=$FILENAME"
