...
do
    ...
    if [[ "$TYPE" == "MSSQL" ]]; then
        SIZE=0
        # Iterate over the subdirectories for each database under the server path
        for DATE in "$TEST_DATE" "$TEST_DATE2" "$TEST_DATE3"; do
            for DB_PATH in $(gsutil ls "gs://$BUCKET/$BACKUP_PATH*/" 2>/dev/null); do
                ...
                for SUB_FOLDER in "DIFF" "FULL"; do
                    echo "Checking path: gs://$BUCKET/$BACKUP_PATH$DB_FOLDER/$SUB_FOLDER/*${DATE}*.bak"
                    FILES=$(gsutil ls "gs://$BUCKET/$BACKUP_PATH$DB_FOLDER/$SUB_FOLDER/*${DATE}*.bak" 2>/dev/null)
                    if [[ -n "$FILES" ]]; then
                        break
                    fi
                done
