...
for DB in $DB_LIST; do
    if [[ "$DB" != "information_schema" ]] && [[ "$DB" != "performance_schema" ]] && [[ "$DB" != "sys" ]] && [[ "$DB" != "mysql" ]]; then
       ...
       DEFAULT NULL,
  `conservation_role`  varchar(100) DEFAULT NULL,
  `GIS_AREA`           float        DEFAULT NULL,
  `GIS_ML_START_DATE`  date         DEFAULT NULL,
  `GIS_ML_END_DATE`    date         DEFAULT NULL,
  PRIMARY KEY (`atu_pk`, `GIS_PRIMARY_SITE`),
  KEY `idx_atu_pk` (`atu_pk`),
  KEY `idx_primary_site` (`GIS_PRIMARY_SITE`)  ) ENGINE=InnoDB "

# This Environment Variable Enables Cleartext Password Plugin
export LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1
