...
function generateQuery {
        ...
        queryStr+="from lgm_daily_backup b "
        queryStr+="join lgm_servers s on s.srv_name = b.ldb_server, "
        queryStr+="(SELECT @rownum := 0) r "
        queryStr+="where ldb_date = cast(now() as date) ${locationConstraint} and s.srv_type='${serverType}'"
        queryStr+="order by s.srv_type desc; "
...
# Generate Queries
#queryOnPrem=$(generateQuery "All" "and srv_location='OnPrem'")
queryMySQL=$(generateQuery "MYSQL" "and srv_location='GCP'")
queryPOSQL=$(generateQuery "POSQL" "and srv_location='GCP'")
queryMSSQL=$(generateQuery "MSSQL" "and srv_location='GCP'")
