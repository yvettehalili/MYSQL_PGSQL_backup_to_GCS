        ...
        for DB_FOLDER in $DB_FOLDERS; do
            ...
            if [[ -n "$FULL_FILES" ]]; then
                ...
                for FILE in $FULL_FILES; do
                    ...
                    if [[ "$FILENAME" =~ ^(sinch-db-cluster\.ti\.ads\$SinchAG.*|sdefryak01)_(.*)_FULL_([0-9]{8})_([0-9]{6})\.bak$ ]]; then
                        DB_NAME="${BASH_REMATCH[2]}"
                        BACKUP_DATE="${BASH_REMATCH[3]}"
                    fi

                    # Convert BACKUP_DATE to the required format `YYYY-MM-DD 00:00:00`
                    BACKUP_DATE_FORMATTED=$(date -d "${BACKUP_DATE}" +"%Y-%m-%d 00:00:00")
