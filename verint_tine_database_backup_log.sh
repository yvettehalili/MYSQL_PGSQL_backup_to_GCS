    ...
    while [[ "$current_date" < "$END_DATE" || "$current_date" == "$END_DATE" ]]; do
        ...
        do
            ...
            for DATE in "$TEST_DATE" "$TEST_DATE2" "$TEST_DATE3"; do
                echo "Checking for files with date: $DATE"
                TEMP_FILES=$(gsutil ls "$BACKUP_PATH/DIFF/*${DATE}*.bak" 2>/dev/null)
                FILES+=$TEMP_FILES
                TEMP_FILES=$(gsutil ls "$BACKUP_PATH/FULL/*${DATE}*.bak" 2>/dev/null)
                FILES+=$TEMP_FILES
                if [[ -n "$FILES" ]]; then
            ...
            for FILE in $FILES; do
                ...
                # Echo and insert details into the backup log
                ...
                # Echo and insert details into the daily log with backup status
