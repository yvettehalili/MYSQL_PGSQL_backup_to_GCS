...
do
    ...
    if [ "$LOCATION" == "OnPrem" ]; then
        ...
        if [ "$TYPE" == "MSSQL" ]; then
            ...
            CHECK=0
            SIZE=0
            SIZE=$(gsutil du -s gs://$BUCKET/Backups/Current/POSTGRESQL/$SERVER/*"${CUR_DATE}"* | awk '{print $1}')
            if [[ $SIZE != *[!\ ]* ]]; then
                CHECK=0
            else
                if [ "$SIZE" != "0" ]; then
                    CHECK=$SIZE;
                    find $STORAGE/Backups/Current/POSTGRESQL/$SERVER/ -type f -name "*${CUR_DATE}*" -print > "/backup/cronlog/$SERVER".txt
                else
                    CHECK=0
                    SIZE=0
                fi
            ...
            if [ "$CHECK" == "0" ]; then
                SIZE=$(gsutil du -s gs://$BUCKET/Backups/Current/POSTGRESQL/$SERVER/*"${CUR_DATE2}"* | awk '{print $1}')
                if [[ $SIZE != *[!\ ]* ]]; then
                    CHECK=0
                else
                    if [ "$SIZE" != "0" ]; then
                        CHECK=$SIZE;
                        find $STORAGE/Backups/Current/POSTGRESQL/$SERVER/ -type f -name "*${CUR_DATE2}*" -print > "/backup/cronlog/$SERVER".txt
                    else
                        CHECK=0
                        SIZE=0
                    fi
            ...
            if [ "$CHECK" == "0" ]; then
                SIZE=$(gsutil du -s gs://$BUCKET/Backups/Current/POSTGRESQL/$SERVER/*"${CUR_DATE3}"* | awk '{print $1}')
                if [[ $SIZE != *[!\ ]* ]]; then
                    CHECK=0
                else
                    if [ "$SIZE" != "0" ]; then
                        CHECK=$SIZE;
                        find $STORAGE/Backups/Current/POSTGRESQL/$SERVER/ -type f -name "*${CUR_DATE3}*" -print > "/backup/cronlog/$SERVER".txt
                    else
                        CHECK=0
                        SIZE=0
                    fi
